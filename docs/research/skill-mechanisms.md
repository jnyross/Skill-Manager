...
Status: complete (both sections); Codex section re-verified 2026-07-08 against a fresh public install (see "Re-verification against codex-cli 0.143.0" below) — the caveat that the original research may reflect a canary build is now discharged.
...
2. **The current official docs' skill-discovery table lists only `.agents/skills` (REPO: `$CWD/.agents/skills`, `$CWD/../.agents/skills`, `$REPO_ROOT/.agents/skills`; USER: `$HOME/.agents/skills`), `/etc/codex/skills` (ADMIN), and a bundled SYSTEM scope — it does not list `~/.codex/skills` as a discovery location for user-authored (non-`.system`) skills at all.** This is in tension with this doc's original framing of `~/.codex/skills` as a "system/legacy" scan location Codex still honors for ordinary skills, and with the bundled skill-creator's own default suggestion ("If you do not have a preference, I will place it in `$CODEX_HOME/skills`... so Codex can discover it automatically") — i.e. Codex's own tooling still defaults new skills into a location its own current public docs no longer document as scanned. Static binary inspection couldn't conclusively resolve whether the live Rust loader still scans arbitrary user content under `~/.codex/skills` (beyond `.system/`) or whether that's now effectively unsupported/undocumented-but-still-functional legacy behavior. Skillet's current dual-root scan (`AgentsHome/skills` and `CodexHome/skills`, `AgentsHome` taking priority on collision) is **not changed** by this finding alone — it's not proven wrong, just less certain than previously stated. Filed as issue #13 for a definitive runtime check (e.g. placing a real skill under each root against a real Codex session and observing which one it picks up).
