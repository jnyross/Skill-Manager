...
The 2026-07-08 implementation conclusion that Codex checks exactly three fixed
candidates is now stale. Codex 0.144.1 source walks every directory between the
repository root and the working directory. An authenticated read-only live
probe on 2026-07-15 confirmed it: from `<repo>/a/b/c`, Codex loaded a uniquely
named skill from `<repo>/a/.agents/skills`, a location outside the old
cwd/immediate-parent/repo-root set. The probe prohibited shell and tool use and
returned the skill's exact sentinel through `codex exec --json --ephemeral
--sandbox read-only`.

Both tools therefore discover project skills through the full inclusive
ancestor chain from the working directory to the repository root, while still
using distinct roots (`.claude/skills` and `.agents/skills`). Skillet's current
three-fixed-candidate Codex inventory behavior must be updated in the curated
setup work; this research correction does not silently alter the shipped code.
